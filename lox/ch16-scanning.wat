    ...
    (:wat::core::do
      ...
      (:wat::core::let
        ...
        (:wat::core::do
          ...
          (:wat::kernel::println "")
          (:wat::kernel::println "The same tokens, checked identical above, at about twice the cost in")
          (:wat::kernel::println "both orderings. The difference is one `defrecord` rebuilt per CHARACTER")
          (:wat::kernel::println "versus one built per TOKEN -- C-098's registerized shape, met again in")
          (:wat::kernel::println "the one place a scanner cannot route around: the innermost loop there")
          (:wat::kernel::println "is. And that is AFTER the update was made as cheap as wat allows: the")
          (:wat::kernel::println "Scanner changes one field with `assoc` rather than restating five, which")
          (:wat::kernel::println "is worth 2.5x -> 1.9x on its own (probes/lox/record-update.wat).")
          (:wat::kernel::println "")
