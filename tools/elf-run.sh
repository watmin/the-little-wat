...
for name in four arith greet branch fib bench strings shadow churn deep logic vectors memory; do
...
echo
if [ $fail -eq 0 ]; then
  echo "elf-run: ok -- nineteen native binaries, seventeen of them compiled from wat source."
  echo "         Thirteen agree with the interpreter; three use syscalls it cannot run (F-119)."
else
  echo "elf-run: FAILED"
fi
exit $fail
