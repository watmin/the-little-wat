...
# that evaluates a source string, not an arbitrary program). This script does those, and then
# the check that matters most: for every program in elf/src/, the COMPILED BINARY and the wat
# INTERPRETER must print exactly the same thing.
#
...
echo
if [ $fail -eq 0 ]; then
  echo "elf-run: ok -- five native binaries, three of them compiled from wat source,"
