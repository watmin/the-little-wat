...
#
# This extracts each routine's hex from elf/lib/runtime.wat and disassembles it, so the claim and
...
grep -oE 'defn :c::rt-[a-z0-9-]+' elf/lib/runtime.wat | sed 's/defn //' | while read -r fn; do
  [ -n "$want" ] && [[ "$fn" != *"$want"* ]] && continue
  hex=$(awk "/defn $fn \[\]/,/^\$/" elf/lib/runtime.wat | grep -oE '"[0-9a-f]+"' | tr -d '"' | tr -d '\n')
  [ -z "$hex" ] && continue
  python3 -c "
import binascii,sys
open('$tmp/r.bin','wb').write(binascii.unhexlify('$hex'))" || continue
  ...
  grep -B4 "defn $fn \[\]" elf/lib/runtime.wat | grep -E '^;; *`' | head -1 | sed 's/^;; */  claim: /'
