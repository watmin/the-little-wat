...
- **Native containers are not affected.** The same test with a `PersistentVector` field:
  `defrecord` is **flat** at ~4000 ns from length 250 to 2000. So a native container is held as a
  handle and a user enum value is cloned — which is why `:wat::grep::Facts` holding five
  `PersistentVector`s is fine, and why this went unnoticed.
