...
### C-053: wat's zero-mutex claim holds — the lost update is unrepresentable
...
- **So Downey's chapter 1 is answered by construction rather than by a semaphore**, and the first
  several puzzles have no wat form because the hazard they remove cannot occur. That is a real
  result about the design, not a gap in the port.
- **Class:** CLEAN.
...
- **A note wat earns against itself:** wat *has* rationals natively, and SICP's hand-built version
  is strictly more capable — `:wat::rational::` is four verbs (`+ - *`, `to-f64`) with no
  division, no comparison and no `to-string` (F-060, C-047).
- **Class:** CLEAN.
...
- **Chapter 13 is where wat has a hard limit, and it is worth stating precisely because C-073 found
  wat *better* equipped than expected on the single-dispatch side.**

### F-109: wat has single dispatch and cannot express multiple dispatch — the method name has no room for the argument types

- **Where:** PAIP chapter 13's generic functions (C-085); probe `probes/paip/multiple-dispatch.wat`.
- **Measured** (wat-rs `a3218644d`, 2026-09-16): a concrete type may extend a surface at exactly
  **one** type parameter. A second extension is refused at startup:
...
- **The error names the cause.** A feature's implementation is registered at
  `<Type>/<feature>` — a name with **no room for the argument types** — so no parameterisation of
  the surface can recover a second dispatch axis. `collide(asteroid, ship)` and
  `collide(asteroid, asteroid)` cannot run different code through the surface mechanism.
- **Single dispatch does work**, and well: two types, two implementations, selected by the receiver
  (C-073 measured this including heterogeneous collections). The gap is specifically the *second
  argument*.
- **The workaround is a table keyed by the tuple of type tags** — which is exactly SICP §2.4's
  data-directed dispatch (C-078), already in this repository. So the honest summary is: **wat has
  the language feature for single dispatch and needs a library pattern for multiple dispatch, and
  the two are not interchangeable.** The table is open and unchecked, and loses precisely the
  exhaustiveness C-078 spent its closing paragraph praising — a missing `(ship, asteroid)` entry is
  a run-time `no-method`, where a missing enum arm is a compile error.
- **Whether this should be fixed is a design question, not a defect report.** Multiple dispatch is
  a large feature and languages reasonably decline it. What is worth having on the record is that
  the *cost* of declining it is exact: every n-ary generic operation becomes a hand-maintained
  table, and the type system stops helping at the door.
- **Class:** EXTEND (multiple dispatch), or CLEAN (say in one line that surfaces dispatch on `self`
  only, and point at the table pattern).
