...
| EOPL (NEXT §9, ~15%) | a slice of ch 3 + ch 5 | **C-061:** Friedman & Wand's one language, two machines. Both agree on arithmetic, `let`, `proc`, `if` and `letrec`. **F-099 lands on the interpreted program**: a direct-style interpreter makes the *interpreted* depth wat's depth, so it segfaults between **40000 and 50000** while the CPS/trampolined machine reaches **300000**. Chapter 5's move is not stylistic in wat — if you write an interpreter here, write the trampolined one. Bonus, found by accident: **wat's TCO is preserved through the direct interpreter** — an interpreted tail call lands in tail position in `value-of`, so my first test could not tell the machines apart at all |
