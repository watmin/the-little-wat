...
| wat-fix (probes + tools) | 16 programs round-tripped | **F-092:** `wat/fix.wat` is the wat-to-wat converter whose header reads "THE PROVING POINT: wat writes wat", and whose documented STASH-DANCE applies it to a whole corpus at once. Converted 16 green self-contained probes and ran both: **7 identical, 8 no longer start, 1 failed to convert.** Four causes — a fn-type bracket gets `wat.core/` where only `wat.type/` resolves; `:wat::WatAST` becomes `wat/WatAST` when **no** symbol spelling of `WatAST` resolves; `Type::method` breaks; a `defenum` name is turned from a keyword into a symbol and the declaration is then refused. **F-093:** and the 7 that converted cleanly each lost call-site type checking. The same program declared `i64 -> i64` and handed a String is **refused at startup** in rust-scheme and **runs, printing the String, exit 0** in faithful-Clojure. Declarations are checked (unknown annotation types and bad return literals are both caught); the argument at the call is not. F-014's consequence, and the codemod's `head-rule` exists to move every call into that spelling |
...
- **Class:** DEFECT. Fix: emit `wat.type/` in a function-type bracket; leave `:wat::WatAST` and
  `Type::method` alone; leave a `defenum` name as a keyword. Or, at minimum, make `fix-text`
  re-read its own output and refuse to emit a form that no longer parses — the machinery to do
  that (`read-string`) is already in the file.
