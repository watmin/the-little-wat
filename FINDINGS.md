...
- **The `slice=1000` row is the one worth keeping.** The same program, with the same bug, is
  *correct* when each thread runs to completion and *loses half its updates* when they interleave.
  Nothing about the program changed — only the scheduler. That is Downey's whole thesis, and the
