...
| Okasaki (NEXT §10, in progress) | ch 2, 3 sets compared | **C-051:** chapter 2's `UnbalancedSet` is 40 lines of wat and correct — recursive parametric enums work, the in-order walk is sorted, and `ok::Set`, `PersistentMap`-to-`true` and `HashSet` agree on all 10000 membership probes over 2000 LCG values (depth 27 vs an optimum of ~11, reported not assumed). **F-097:** and it is **34x slower to build, 43x slower to query** than the workaround F-057 forces. `BASELINE.md` predicts why to within 5%: 1922 ns per node visited against a predicted 1830–2190 for one call + one match + a comparison. So F-057's gap needs a **native** persistent set; a library one cannot win, and `PersistentMap`-to-`true` is the right answer until there is one |
