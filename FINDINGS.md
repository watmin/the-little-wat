...
### F-123: `Vector/conj` clones unconditionally, even when the `Arc` is uniquely referenced — `Arc::make_mut` is the fix, and wat-rs already uses it elsewhere

- **Where:** `wat-rs/src/collection/eval.rs:280`, `vector_conj_inner`. Found while asking whether a
  compiled wat needs a garbage collector (C-126).
- **What it does now:**
  ```rust
  Value::Vec(xs) => {
      let mut out = (**xs).clone();       // always, whatever the refcount is
      out.push(item.clone());
      Ok(Value::Vec(Arc::new(out)))
  }
  ```
  A full copy of the vector on every `conj`, even when this `Arc` is the only reference to it and
  nothing could observe the mutation.
- **`Arc::make_mut` is exactly that check** — clone only if shared, otherwise hand back `&mut` —
  and **wat-rs already uses it in 27 places**, just not on any collection `conj` path. In the
  accumulator loop, which is the idiomatic way to build a collection in wat, the refcount is 1
  every time, so every one of those clones is avoidable.
- **Measured** (this machine, best of 3, 20000 `conj`s onto an accumulator):

  | | time |
  | --- | --- |
  | `Vector` | 4142 ms |
  | `PersistentVector` | **655 ms** |

  F-116 measured the same shape at depth 4000 (121 ms vs 34 ms). `PersistentVector` wins because
  its structural sharing makes the copy O(log n) — but `make_mut` would make `Vector`'s copy
  **disappear entirely** in this shape, which is better than either.
- **The memory is already fine**, which is what makes this purely a time cost worth reclaiming:
  the interpreter's peak RSS over that loop grows *linearly* (70.4 MB at n=5000 → 79.5 MB at
  n=40000, about 0.2 KB per element) because each dead clone's refcount hits zero and `Drop`
  frees it immediately. So `conj` is O(n) in time and O(1) in retained memory; only the time is
  being thrown away.
- **Class:** IMPROVE. `Arc::make_mut` in `vector_conj_inner`, and the same question for
  `list_conj_inner`, `hashset_conj_inner` and `assoc`.
- **Repro:** the two programs in C-126's entry, or F-116's.
...
- **What to build, in order.** (1) Refcount in the object header, since it matches what wat
  already does and needs no language change: an inc where a pointer is shared, a dec where one
  dies, and a free list. (2) In-place update when the count is 1 — Rust's `Vec::push`, Clojure's
  transient, Swift's `isKnownUniquelyReferenced` — which makes the accumulator loop O(n) in time
  as well as memory. wat-rs does **not** do (2) even though it could; that is **F-123**.
